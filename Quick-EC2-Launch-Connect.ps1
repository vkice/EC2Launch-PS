try {
    ...
    aws ec2 create-key-pair --key-name $keyinput --key-format pem --query 'KeyMaterial' --region $region --output text | out-file -encoding ascii -filepath $PSScriptRoot\$newfolder\$keyinput.pem
    ...
    aws ec2 run-instances --image-id $inputami --count 1 --instance-type t2.micro --key-name $keyinput --region $region | Out-File -encoding ascii -filepath $PSScriptRoot\$newfolder\$output.txt
    ...
    while ($after1 -eq "launch") {
        $after2 = Read-Host -Prompt "New Powershell has been launched, command has been copied to your below and is shown below in case of disconnect:`n
        ...
        Press enter to regenerate the new Powershell console, public EC2 DNS name, and SSH command.`n"
        Set-Clipboard -Value "ssh -i $PSScriptRoot\$newfolder\$keyinput.pem -l ec2-user $publicip"
